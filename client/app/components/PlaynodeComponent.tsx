			...
			<Handle type="target" isConnectableStart={false} position={Position.Top} style={{ width: 12, height: 12 }} />
			{
				props.selected ?
					<div className={`border-${color}-600 bg-${color}-100 border-4 rounded-xl w-64 p-4 text-${color}-600`} onDrop={handleDrop} onDragOver={e => e.preventDefault()}>
						<div className="mb-5">
							<button className={`bg-${color}-300 rounded-lg px-2 py-1 absolute left-1 top-1`} onClick={handleTogglePlaynodeType} title={props.data.playnode.type}>{isSequence ? <>🔢</> : <>🎲</>}</button>
							<button className={`bg-blue-300 rounded-lg px-2 py-1 absolute left-10 top-1`} onClick={handleToggleScope} title={scopeView ? "Toggle Song View" : "Toggle Scope View"}>{scopeView ? <>🎶</> : <>🔲</>}</button>
							<button className={`bg-red-300 rounded-lg px-2 py-1 absolute right-1 top-1`} onClick={handleDeleteSelf} title="Delete Playnode">🗑️</button>
						</div>
